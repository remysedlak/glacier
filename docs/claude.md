...
- `app/click.rs` — ClickResult enum + handle_click_result (moved here from graphics/mod.rs to sit next to its handler, matching UiCommand's placement convention)
...
- Returns ClickResult::ToggleTrackMute(i)
...
- ID minting from `.len()` collides after deletion, across tracks/patterns/events alike — use `.iter().map(|x| x.id).max().map(|m| m+1).unwrap_or(0)` at every creation site instead. Display labels (like "Pattern N") can still use `.len()`, since they're just a count, not an identity.
...
- **`ResizeAudioBlock` never notifies the UI.** `audio.rs`'s handler mutates its
  own `events` copy but sends no `UiCommand` back — `gfx.events` never learns a
  resize happened, so the UI-side length can drift stale from the audio
  thread's real value until something else forces a full resync (e.g. a save
  and reload doesn't help either, since the save writes the audio thread's
  correct value but the *running* UI display doesn't refresh from it). Needs a
  new `UiCommand::AudioBlockResized(usize, u32)` and a receiving arm that
  updates the matching `gfx.events` entry (likely via `load_event` once that's
  rewritten — see next item).
- **`Graphics::load_event` is dead/unused.** Exists as a bare `.push()` with no
  update-or-insert logic; `AudioBlockLoaded`'s handler pushes into `gfx.events`
  directly instead of calling it. Should be rewritten to mirror `load_pattern`'s
  shape (find-by-id, overwrite if present, else push) once `ResizeAudioBlock`
  needs update-or-insert — that's the second real call site that would justify
  the rewrite.
- **`Graphics::load_track` is dead code.** `UiCommand::TrackLoaded`'s handler
  now does `gfx.tracks.push(track)` directly instead of calling
  `gfx.load_track(track)`. The wrapper's old guard (`if i.data.id >=
  self.tracks.len() as u32`) was never proven safe or unsafe before it stopped
  being called — worth deleting the dead method, or tracing through whether it
  was ever actually needed.
- **`playlist.rs`'s length-computation match still has `_ => 1`** for the
  `Sample` arm — the original ask that started this whole session. Dropping a
  file onto the playlist still always creates a 1-step-long block regardless of
  the actual sample's duration. Needs: resolve the track's sample count via
  `tracks.iter().find(|t| t.data.id as usize == *id)`, convert frame count →
  step count via `glacier_dsp::samples_per_step(sample_rate, bpm)` with a
  ceiling division (not truncating). Waveform-inside-the-block rendering is a
  separate, second problem — reuse the reduction math (mono-down → stride →
  per-stride min/max) already written for the track detail window's waveform,
  parameterized by width instead of hardcoded to `TRACK_GRAPHICS_WIDTH`.
- **`track.rs`'s waveform drawing has known latent bugs**, not yet triggered by
  any file tested so far: `for pixel_column in 0..199` should be `0..200`
